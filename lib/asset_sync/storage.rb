...
module AssetSync
  class Storage
    ...
    def initialize(cfg)
      puts "INVOKING: #{self.class.name} => def initialize(cfg)"
      @config = cfg
    end

    def connection
      puts "INVOKING: #{self.class.name} => def connection"
      @connection ||= Fog::Storage.new(self.config.fog_options)
    end

    def bucket
      puts "INVOKING: #{self.class.name} => def bucket"
    ...
    def keep_existing_remote_files?
      puts "INVOKING: #{self.class.name} => def keep_existing_remote_files?"
      self.config.existing_remote_files?
    end

    def path
      puts "INVOKING: #{self.class.name} => def path"
      self.config.public_path
    end

    def remote_file_list_cache_file_path
      puts "INVOKING: #{self.class.name} => def remote_file_list_cache_file_path"
      self.config.remote_file_list_cache_file_path
    end

    def ignored_files
      puts "INVOKING: #{self.class.name} => def ignored_files"
      expand_file_names(self.config.ignored_files)
    end

    def get_manifest_path
      puts "INVOKING: #{self.class.name} => def get_manifest_path"
    ...
    def local_files
      puts "INVOKING: #{self.class.name} => def local_files"
      @local_files ||=
        (get_local_files + config.additional_local_file_paths).uniq
    end

    def remote_files
      puts "INVOKING: #{self.class.name} => def remote_files"
    ...
    def update_remote_file_list_cache(local_files_to_upload)
      puts "INVOKING: #{self.class.name} => def update_remote_file_list_cache(local_files_to_upload)"
    ...
    def get_asset_files_from_manifest
      puts "INVOKING: #{self.class.name} => def get_asset_files_from_manifest"
    ...
    def get_local_files
      puts "INVOKING: #{self.class.name} => def get_local_files"
    ...
    def get_remote_files
      puts "INVOKING: #{self.class.name} => def get_remote_files"
    ...
    def delete_file(f, remote_files_to_delete)
      puts "INVOKING: #{self.class.name} => def delete_file(f, remote_files_to_delete)"
    ...
    def delete_extra_remote_files
      puts "INVOKING: #{self.class.name} => def delete_extra_remote_files"
    ...
    def upload_file(f)
      puts "INVOKING: #{self.class.name} => def upload_file(f)"
    ...
    def upload_files
      puts "INVOKING: #{self.class.name} => def upload_files"
      # fixes: https://github.com/rumblelabs/asset_sync/issues/19
    ...
    def ignore_existing_remote_files?
      puts "INVOKING: #{self.class.name} => def ignore_existing_remote_files?"
      self.config.existing_remote_files == 'ignore'
    end

    def get_non_fingerprinted(files)
      puts "INVOKING: #{self.class.name} => def get_non_fingerprinted(files)"
      files.map do |file|
    ...
    def expand_file_names(names)
      puts "INVOKING: #{self.class.name} => def expand_file_names(names)"
      files = []
