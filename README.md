...
If you want download a manifest file and asset files on multi servers without committing to your repository, you can use `assets:sync:download` rake task:
...
`assets:sync:manifest` rake task will download the manifest file from your cloud storage. `include_manifest` config option must be set as `true`.

`assets:sync:asset_files` rake task will download asset files from your cloud storage. If you set `manifest` config option as `true`, the task will download files mentioned in the manifest. Otherwise, it will download all remote files.
